...
class Assets(WMLResource):
                    ...
                    if connection_id is None:
                        ...
                        if put_response.status_code == 201 or put_response.status_code == 200:
                            ...
                            complete_response = requests.post(
                                self._client.service_instance._href_definitions.get_attachment_complete_href(asset_id,
                                                                                                             attachment_id),
                                headers=self._client._get_headers(),
                                params=self._client._params()
    ...
    def list(self, limit=None, return_as_df=True):
        ...
        :type limit: int
        :param return_as_df: determine if table should be returned as pandas.DataFrame object, default: True
        ...
        **Example**
        ...
        space_values = [
            (m[u'metadata'][u'name'], m[u'metadata'][u'asset_type'], m[u'metadata'][u'size'], m["metadata"]["asset_id"])
            for m in asset_details]

        table = self._list(space_values, [u'NAME', u'ASSET_TYPE', u'SIZE', u'ASSET_ID'], limit, _DEFAULT_LIST_LENGTH)
        if return_as_df:
            return table
        ...
        **Example**
        ...
        if self._WSD:
            ...
            response = requests.get(
                self._client.service_instance._href_definitions.get_attachment_href(asset_uid, attachment_id),
                params=self._client._params(),
                headers=self._client._get_headers())
            if response.status_code == 200:
                if 'connection_id' in asset_details["attachments"][0] and \
                    ...
                    cos_conn_data_source_id = self._client.connections.get_datasource_type_uid_by_name(
                        'cloudobjectstorage')
                ...
                if att_response.status_code != 200:
                    raise ApiRequestFailure(Messages.get_message(message_id="failure_during_downloading_data_asset"),
                                            att_response)
