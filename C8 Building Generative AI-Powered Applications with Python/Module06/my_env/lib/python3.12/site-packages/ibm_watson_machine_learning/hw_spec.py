...
class HwSpec(WMLResource):
    ...
    def store(self, meta_props):
        ...
        **Example**
        ...
        hw_spec_details = self._handle_response(201, u'creating hardware specification', creation_response)

        return hw_spec_details

    def list(self, name=None, return_as_df=True, limit=None):
        ...
        **Example**
        ...
        hw_spec_values = [
            ...
            m in asset_details]
        table = self._list(hw_spec_values, [u'NAME', u"ID", u'DESCRIPTION'], limit, _DEFAULT_LIST_LENGTH)
        if return_as_df:
            return table
    ...
    def delete(self, hw_spec_id: str):
        """Delete a hardware specifications.

        :param hw_spec_id: Unique Id of hardware specification  which should be deleted
        :type hw_spec_id: str

        :return: status ("SUCCESS" or "FAILED")
        :rtype: str
        """
        HwSpec._validate_type(hw_spec_id, u'hw_spec_id', str, True)

        response = requests.delete(self._client.service_instance._href_definitions.get_hw_spec_href(hw_spec_id), params=self._client._params(),
                                headers=self._client._get_headers())
