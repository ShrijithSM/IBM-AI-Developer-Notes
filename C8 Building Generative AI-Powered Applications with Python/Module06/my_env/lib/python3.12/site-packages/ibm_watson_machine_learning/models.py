...
from ibm_watson_machine_learning.metanames import ModelMetaNames, LibraryMetaNames, _check_spec_uids
from ibm_watson_machine_learning.utils.autoai.utils import (download_request_json, load_file_from_file_system_nonautoai, 
                                                            init_cos_client, check_if_ts_pipeline_is_winner, 
...
from ibm_watson_machine_learning.wml_client_error import WMLClientError, ApiRequestFailure, UnexpectedType, MissingValue
...
class Models(WMLResource):
        ...
        if "pipeline" in details["entity"] or "experiment" in details["entity"]:
            if "experiment" in details["entity"]:
                ...
                if self.ConfigurationMetaNames.RUNTIME_UID not in meta_props and not self._client.CLOUD_PLATFORM_SPACES \
                        and not self._client.CPD_version:
                    model_meta.update({"runtime": {"href": "/v4/runtimes/" + runtime_uid}})
        ...
        if self._client.CLOUD_PLATFORM_SPACES or self._client.CPD_version:
            ...
            if self._client.ICP_PLATFORM_SPACES:
                ...
                if len(details['entity']['results_reference']['connection']) > 1:
                    ...
                    DataConnection = _check_if_import_from_watsonx_ai(self.__module__,
                                                                      'ibm_watson_machine_learning.helpers', 
                                                                      'DataConnection')
    ...
    def _publish_from_archive(self, path_to_archive, meta_props=None,version=False,artifactid=None, feature_names=None, label_column_names=None):
        ...
        url = self._client.service_instance._href_definitions.get_published_models_href()
        if self._client.CLOUD_PLATFORM_SPACES or self._client.CPD_version:
            payload = self._create_cloud_model_payload(meta_props, feature_names=feature_names,
                                                       label_column_names=label_column_names)
            retry = True
            while retry:
                retry = False
                ...
                if response.status_code == 400 and "hybrid_pipeline_software_specs" in response.text and "hybrid_pipeline_software_specs" in payload:
    ...
    def store(self, model=None, meta_props=None, training_data=None, training_target=None, pipeline=None, version=False,
        ...
        **Examples**
        ...
        if ((self._client.CLOUD_PLATFORM_SPACES or self._client.CPD_version) and self._client.default_space_id is None
        ...
        is_training_prompt_tuning = _check_if_import_from_watsonx_ai(self.__module__,
                                                                     'ibm_watson_machine_learning.foundation_models.utils.utils', 
                                                                     'is_training_prompt_tuning')
        custom_model = False
        if (isinstance(meta_props, dict) and
                meta_props.get(self.ConfigurationMetaNames.TYPE, '').startswith('custom_foundation_model')):
            if self._client.CLOUD_PLATFORM_SPACES or self._client.CPD_version < 4.8:
                raise WMLClientError(Messages.get_message('>= 4.8', message_id="invalid_cpd_version"))
            
            custom_model = True

        Models._validate_type(model, u'model', object,
                            mandatory=not is_training_prompt_tuning(training_id, wml_client=self._client))

        if is_training_prompt_tuning(training_id, wml_client=self._client):
            Models._validate_type(meta_props, u'meta_props', [dict, str], mandatory=False)
            if meta_props is None:
                meta_props = {}  # for backward compatibility and future checks
        else:
            Models._validate_type(meta_props, u'meta_props', [dict, str],
                                  mandatory=is_training_prompt_tuning(training_id, wml_client=self._client))
        ...
        if self._client.CAMS and not self._client.CPD_version:
            if self._client.default_space_id is not None:
                meta_props.update(
        ...
        if training_id and is_training_prompt_tuning(training_id, self._client):
            ...
            load_request_json = _check_if_import_from_watsonx_ai(self.__module__,
                                                                 'ibm_watson_machine_learning.foundation_models.utils.utils', 
                                                                 'load_request_json')

            model_request_json = load_request_json(run_id=training_id, wml_client=self._client)
            if meta_props:
        ...
        if model is not None and custom_model:
                ...
                if self._client.CLOUD_PLATFORM_SPACES or self._client.CPD_version:
                    if experiment_metadata or training_id:
                        if experiment_metadata:
                            get_autoai_run_id_from_experiment_metadata = _check_if_import_from_watsonx_ai(self.__module__,
                                                                                                          'ibm_watson_machine_learning.utils.autoai.utils',
                        ...
                        prepare_auto_ai_model_to_publish  =_check_if_import_from_watsonx_ai(self.__module__,
                                                                                            'ibm_watson_machine_learning.utils.autoai.utils',
                        ...
                        request_json = download_request_json(run_params=run_params,
                                                             model_name=self._get_last_run_metrics_name(training_id=training_id),
                                                             wml_client=self._client,
                        ...
                        if request_json:
                            ...
                            if "hybrid_pipeline_software_specs" in request_json:
                                new_meta_props["hybrid_pipeline_software_specs"] = request_json["hybrid_pipeline_software_specs"]
                        ...
                        if experiment_metadata:
                            ...
                            if 'training_data_references' in experiment_metadata:
                                new_meta_props[self._client.repository.ModelMetaNames.TRAINING_DATA_REFERENCES] \
                                    = [e._to_dict() if isinstance(e, DataConnection) else e for e in experiment_metadata.get('training_data_references')]
                            ...
                            if 'test_data_references' in experiment_metadata:
                                new_meta_props[self._client.repository.ModelMetaNames.TEST_DATA_REFERENCES] \
                                    = [e._to_dict() if isinstance(e, DataConnection) else e for e in experiment_metadata.get('test_data_references')]
                        else:  # if training_id
                            label_column = None
    ...
    def update(self, model_uid, meta_props=None, update_model=None):
        ...
        :param update_model: archived model content file or path to directory containing archived model file
            which should be changed for specific model_uid
    ...
    def load(self, artifact_uid):
        ...
        **Example**
        ...
        if self._client.CAMS or self._client.CLOUD_PLATFORM_SPACES or self._client.CPD_version:
    ...
    def download(self, model_uid, filename='downloaded_model.tar.gz', rev_uid=None, format=None):
        ...
        **Example**
        ...
        if rev_uid and isinstance(rev_uid, int):
        ...
        try:
            if self._client.CLOUD_PLATFORM_SPACES or self._client.ICP_PLATFORM_SPACES:
                ...
                if rev_uid is not None:
                    params.update({'revision_id': rev_uid})
                if not self._client.CPD_version:
    ...
    def get_revision_details(self, model_uid, rev_uid):
        ...
        **Example**
        ...
        self._client._check_if_either_is_set()
        Models._validate_type(model_uid, u'model_uid', str, True)
        if not Models._validate_type(rev_uid, u'rev_uid', [str, int], True):
        ...
        if 'scikit-learn_' in model_type or 'mllib_' in model_type:
            ...
            with open(path_to_archive, 'rb') as f:
                if is_xml(path_to_archive):
                    ...
                    if (self._client.CLOUD_PLATFORM_SPACES or self._client.CPD_version) and model_type.startswith('wml-hybrid_0'):
                        response = self._upload_autoai_model_content(f, url, qparams)
                    else:
                        if self._client.CLOUD_PLATFORM_SPACES or self._client.CPD_version:
                            qparams.update({'content_format': "native"})
    ...
    def _publish_from_object_cloud(self, model, meta_props, training_data=None, training_target=None, pipeline=None,
        ...
        try:
            ...
            if str(meta_props[self.ConfigurationMetaNames.TYPE]).startswith("tensorflow_") and \
                    (self._client.ICP_PLATFORM_SPACES or self._client.CLOUD_PLATFORM_SPACES):
    ...
    def _store_custom_foundation_model(self, model, meta_props):
    ...
    def _create_custom_model_payload(self, model, meta_props):
        metadata = copy.deepcopy(meta_props)

        Models._validate_type(model, u'model_id', str, True)
        ...
        metadata.update({self.ConfigurationMetaNames.FOUNDATION_MODEL: {'model_id': model}})
    ...
    def _get_last_run_metrics_name(self, training_id):
        run_metrics = self._client.training.get_metrics(training_uid=training_id)
        last_run_metric = run_metrics[-1]
        metrics_name = last_run_metric.get('context').get('intermediate_model').get('name')
        return metrics_name
