...
class HrefDefinitions:
    ...
    def get_fm_specifications_href(self, limit):
        if self._client._use_fm_ga_api:
            ...
            return FM_GET_SPECS_BETA.format(self._wml_credentials['url'], limit)  # Remove on CPD 5.0 release

    def get_fm_tasks_href(self, limit):
        return FM_GET_TASKS.format(self._wml_credentials['url'], limit)

    def get_fm_generation_href(self, item=None):
        if self._client._use_fm_ga_api:
            return FM_GENERATION.format(self._wml_credentials['url'])
        else:
            return FM_GENERATION_BETA.format(self._wml_credentials['url'], item)  # Remove on CPD 5.0 release

    def get_fm_generation_stream_href(self):
        return FM_GENERATION_STREAM.format(self._wml_credentials['url'])

    def get_fm_tokenize_href(self):
        if self._client._use_fm_ga_api:
            return FM_TOKENIZE.format(self._wml_credentials['url'])
        else:
            return FM_TOKENIZE_BETA.format(self._wml_credentials['url'])  # Remove on CPD 5.0 release

    def get_fm_deployment_generation_href(self, deployment_id, item=None):
        if self._client._use_fm_ga_api:
            return FM_DEPLOYMENT_GENERATION.format(self._wml_credentials['url'], deployment_id)
        else:
            return FM_DEPLOYMENT_GENERATION_BETA.format(self._wml_credentials['url'], deployment_id, item)  # Remove on CPD 5.0 release

    def get_fm_deployment_generation_stream_href(self, deployment_id):
        return FM_DEPLOYMENT_GENERATION_STREAM.format(self._wml_credentials['url'], deployment_id)
