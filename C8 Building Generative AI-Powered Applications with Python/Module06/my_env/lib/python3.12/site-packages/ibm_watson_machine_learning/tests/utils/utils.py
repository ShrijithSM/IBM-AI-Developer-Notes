...
import ibm_boto3
from ibm_boto3 import s3

from ibm_watson_machine_learning.helpers.connections.connections import ContainerLocation
...
def delete_container(wml_client, cos_path_data):
    import ibm_watson_machine_learning._wrappers.requests as requests
    ...
    cos_resource = ibm_boto3.resource(
        ...
        aws_access_key_id=properties.get('access_key_id'),
        aws_secret_access_key=properties.get('secret_access_key'),
    )

    bucket_object_list = cos_resource.Bucket(properties['bucket_name']).objects.all()

    for x in bucket_object_list:
        if cos_path_data in x.key:
            status = x.delete()
            print(f"\nDeleted: {x}")
            return status
...
def get_available_models():
    ...
    available_models = [model_spec['model_id'] for model_spec in
                        get_model_specs(wml_credentials.get('url')).get('resources', [])
