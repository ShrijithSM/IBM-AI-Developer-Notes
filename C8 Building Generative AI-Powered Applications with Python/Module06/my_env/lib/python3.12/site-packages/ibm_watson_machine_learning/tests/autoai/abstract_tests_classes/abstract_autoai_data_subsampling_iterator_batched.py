...
from ibm_watson_machine_learning.tests.utils import create_connection_to_cos, get_cos_credentials, \
    get_space_id
            ...
            for return_data_as_iterator in [True, False] if self.is_non_iterator_available() else [True]:
                for enable_sampling in [True, False]:
                    for sampling_type in [None, SamplingTypes.RANDOM,
                                          SamplingTypes.STRATIFIED] if enable_sampling or not return_data_as_iterator else [
                        None]:
                        for total_size_limit in [None, 2 * 1024 * 1024, 25 * 1024 * 1024] if enable_sampling else [
                            ...
                            for total_nrows_limit in [2000, None] if enable_sampling else [None]:
                                for number_of_batch_rows in [None] if total_size_limit or enable_sampling else [None,
                                                                                                                1000]:
                                    for sample_size_limit in [None] if number_of_batch_rows else [None,
                                                                                                  2 * 1024 * 1024]:
