...
from ibm_watson_machine_learning.tests.utils.assertions import validate_autoai_timeseries_experiment
...
class AbstractTestTSAsync(abc.ABC):
    ...
    def test_08c_get_pipeline_notebook(self):
        ...
        details = self.wml_client.training.get_details(
            training_uid=self.remote_auto_pipelines.get_run_details()['metadata']['id'])
        winners = summary._series['Enhancements'].keys()
        no_winner_pipelines = ['Pipeline_' + m['context']['intermediate_model']['name'][1:] for m in
                               details['entity']['status']['metrics'] if
                               'Pipeline_' + m['context']['intermediate_model']['name'][1:] not in winners]
    ...
    def test_99a_delete_used_assets(self):
        file_name = AbstractTestTSAsync.batch_output_filename
        print("\n=========FILE NAME=========")
        print(file_name)
