...
from ibm_watson_machine_learning.tests.utils import (get_cos_credentials, get_space_id)
...
class AbstractTestAutoAIAsync(abc.ABC):
    ...
    def test_01_initialize_AutoAI_experiment__pass_credentials__object_initialized(self):
        ...
        AbstractTestAutoAIAsync.batched = (
                                                      'include_batched_ensemble_estimators' not in self.experiment_info and self.wml_client.ICP) or \
    ...
    def test_03_initialize_optimizer(self):
        ...
        AbstractTestAutoAIAsync.remote_auto_pipelines = self.experiment.optimizer(
            **AbstractTestAutoAIAsync.experiment_info)
    ...
    def test_08c_get_pipeline_notebook(self):
        ...
        if self.batched:
            ...
            test_notebook(ensemble_pipeline_name, 'test_notebook_ensemble.ipynb',
                          additional_check=check_if_ensemble_content)
            ...
            for score_type in sort_by_holdout.keys():
                for scoring in scorers[prediction_type]:
                    pipelines_summary = self.remote_auto_pipelines.summary(scoring=scoring,
                                                                           sort_by_holdout_score=sort_by_holdout[
                                                                               score_type])
                    print(pipelines_summary.to_string())
                    ...
                    if scoring == self.remote_auto_pipelines.params['scoring'] and score_type == 'training':
                        scoring += '_(optimized)'
                    if is_regression:
                        metric_values = pipelines_summary[
                            f"{score_type}_{scoring[4:]}"]  # take score name without 'neg_'
                    else:
...
class AbstractTestAutoAISync(abc.ABC):
    ...
    def test_03_initialize_optimizer(self):
        ...
        AbstractTestAutoAISync.remote_auto_pipelines = self.experiment.optimizer(
    ...
    def test_55_waiting_for_fitted_completed(self):
        ...
        self.assertEqual(status, RunStateTypes.COMPLETED,
                         msg=f"AutoAI run didn't finished successfully. Status: {status}\n"
                             f" Run Details: {self.remote_auto_pipelines.get_run_details()}")
