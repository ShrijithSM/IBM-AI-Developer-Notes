...
class TestScikitLearnDeployment(AbstractOnlineDeploymentTest, unittest.TestCase):
    ...
    def get_package_extension_file(self):
        return os.path.join(os.getcwd(), 'base', 'artifacts', 'other', 'pkg_extension.yml')
    ...
    def create_package_extension(self):
        meta_prop_pkg_extn = {
            ...
            self.wml_client.package_extensions.ConfigurationMetaNames.TYPE: "conda_yml"
        }
        ...
        self.wml_client.package_extensions.download(pkg_extn_uid, "./test_pkg_extension.yml")

        self.assertTrue(filecmp.cmp('./test_pkg_extension.yml', self.get_package_extension_file()))

        return pkg_extn_uid

    def create_software_spec_with_pkg_extension(self, pkg_extn_id):
        sw_spec_meta_props = {
            self.wml_client.software_specifications.ConfigurationMetaNames.NAME: "runtime-23.1-py3.10 with test pkg_extension",
