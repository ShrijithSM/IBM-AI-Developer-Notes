...
class PlatformSpaces(WMLResource):
    ...
    def _connection_validation(self):
        """Trial connection to validate authorization.

        :raises WMLClientError: raises when connection is unauthorized
        """        
        href = self._client.service_instance._href_definitions.get_platform_spaces_href()
        response_get = requests.get(href,
                                    headers=self._client._get_headers(),
                                    params={"limit": 1})
        if response_get.status_code == 401:
            raise WMLClientError(Messages.get_message(response_get.json()['errors'][0]['message'],
    ...
    def list(self, limit=None, member=None, roles=None, return_as_df=True, space_type=None):
        ...
        :param space_type: filter spaces by their type; available types: 'wx', 'cpd', 'wca'
        ...
        **Example**
        ...
        space_resources = [m for r in self._get_with_or_without_limit(href,
                                                          None,
                                                          'spaces',
                                                          summary=False,
                                                          pre_defined=False,
