...
from ibm_watson_machine_learning.metanames import ScriptMetaNames, _check_spec_uids
    ...
    def _create_asset(self, script_meta, file_path, extension='.py'):
        ...
        if not self._client.WSD:
            ...
            if lang == 'R':
                ...
                warning_error_msg = ("Provided software spec is {key} for R scripts. " +
                ...
                for key in unsupported_dict:
                    if script_meta[u'software_spec_uid'] in unsupported_dict[key]:
                        match key:
                            ...
                            case _:
                                warnings.warn(warning_error_msg.format(key=key), category=RuntimeWarning)
            ...
            elif sw_spec_details[u'entity'][u'software_specification'][u'type'] == 'derived':
                asset_meta = {
                    "metadata": {
                        "name": name,
                        "description": desc,
    ...
    def download(self, asset_uid, filename, rev_uid=None):
        ...
        **Example**
        ...
        if rev_uid is not None:
            if not Script._validate_type(rev_uid, u'rev_id', [str, int], False):
    ...
    def get_revision_details(self, script_uid, rev_uid=None):
        ...
        **Example**
        ...
        if rev_uid is None:
            ...
            if not Script._validate_type(rev_uid, u'rev_uid', [str, int], False):
                raise UnexpectedType('rev_uid', [str, int], type(rev_uid))
            elif isinstance(rev_uid, int):
                warnings.warn("Parameter rev_uid as int is depreacted, should be of type str.")
