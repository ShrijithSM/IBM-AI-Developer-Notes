...
class Pipelines(WMLResource):
    ...
    def update(self, pipeline_uid, changes, rev_id=None):
        ...
        **Example**
        ...
        response = requests.patch(url, json=patch_payload, params=self._client._params(),
                                  headers=self._client._get_headers())
    ...
    def delete(self, pipeline_uid):
        ...
        **Example**
        ...
        response_delete = requests.delete(pipeline_endpoint, params=self._client._params(),
                                          headers=self._client._get_headers())
    ...
    def get_details(self, pipeline_uid=None, limit=None, asynchronous=False, get_all=False):
        ...
        **Example**
        ...
        if self._client.WSD:
            ...
            response = self._get_artifact_details(url, pipeline_uid, limit, 'pipeline' if pipeline_uid else 'pipelines',
                                                  summary="False")
    ...
    def get_revision_details(self, pipeline_uid, rev_uid):
        ...
        **Example:**
        ...
        .. note::
         ...
         """
        Pipelines._validate_type(pipeline_uid, u'pipeline_uid', str, True)
        if not Pipelines._validate_type(rev_uid, u'rev_uid', [str, int], True):
            raise UnexpectedType('rev_uid', [str, int], type(rev_uid))
        elif isinstance(rev_uid, int):
            from warnings import warn
            warn("Parameter rev_uid as int is depreacted, should be of type str.")
        if not self._client.CLOUD_PLATFORM_SPACES and not self._client.CPD_version:
            raise WMLClientError(
        ...
        **Example**
        ...
        if 'asset_type' in pipeline_details['metadata']:
            ...
            if 'href' in pipeline_details['metadata']:
                ...
                pipeline_id = WMLResource._get_required_element_from_dict(pipeline_details, u'pipeline_details',
                                                                          [u'metadata', u'id'])
        ...
        **Example**
        ...
        if self._client.WSD:
            ...
            if self._client.CLOUD_PLATFORM_SPACES or self._client.CPD_version:
                ...
                pipeline_values = [(m[u'metadata'][u'id'], m[u'entity'][u'name'], m[u'metadata'][u'created_at']) for m
                                   in pipeline_resources]
