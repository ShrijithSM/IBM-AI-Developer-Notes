...
class CryptoFHE(Crypto):
    ...
    def initialize(self, config):
        ...
        if self.private_fusion_weight is None:
            logger.info("No flag for privacy of fusion weights in config. " "Setting to default value of False.")
    ...
    def aggregate_collected_ciphertext_response(self, lst_model_updates, key="ct_weights", **kwargs):
        ...
        if not self.private_fusion_weight:
            ...
            for model_update in lst_model_updates:
                ...
                if cur_weights_dtype is not None:
                    if weights_dtype is None:
                        ...
                        logger.warning(
                            "aggregate_collected_ciphertext_response: weights dtype mismatch: "
                            + str(weights_dtype)
                            + " != "
                            + str(cur_weights_dtype)
                        )
            ...
            ct_fused_weights = self.cipher.fuse_weights(lst_ct_weights, lst_ct_counts, low_bound, up_bound, eps)
...
class CipherFHE(Cipher):
    ...
    def encrypt_weights(self, model_weights, **kwargs):
        ...
        for i in range(len(model_weights)):
            ...
            if self.weights_dtype is None:
                ...
                logger.warning(
                    "encrypt_weights: weights dtype for multiple layers is different: "
        ...
        if counts is not None:
            ...
            ct_counts = self.crypto.encode_encrypt(shape_counts, np.array(counts).reshape(1, 1))
    ...
    def decrypt_weights(self, ct_model_weights, **kwargs):
        ...
        if self.weights_dtype is None:
            ...
            if weights_dtype is not None:
                ...
                logger.info("decrypt_weights: weights dtype obtained from incoming model: " + str(self.weights_dtype))
            else:
                self.weights_dtype = np.zeros(1, np.float32).dtype
                logger.warning(
                    "decrypt_weights: weights dtype is None. " "Using default dtype: " + str(self.weights_dtype)
                )
    ...
    def fuse_weights_scalar(self, ct_lst_weights, lst_fusion_weight):
        ...
        if len(lst_fusion_weight) != len(ct_lst_weights):
            raise CryptoException("Size of fusion weight does not match" "the size of recevied ciphertext list")
        ...
        for layer_idx in range(len(fused_ct_keras)):
            ...
            for party_idx in range(1, len(ct_lst_weights)):
                ...
                party_ct.load_from_buffer(ct_lst_weights[party_idx][layer_idx][0])
    ...
    def fuse_weights(self, ct_lst_weights, ct_lst_fusions, low_bound, up_bound, eps):
        """
        Fuse a set of encrypted model weights with plaintext fusion
        weight for each model.
