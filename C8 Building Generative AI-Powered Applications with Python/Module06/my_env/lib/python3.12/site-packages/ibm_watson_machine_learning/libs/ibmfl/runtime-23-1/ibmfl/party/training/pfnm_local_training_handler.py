...
class PFNMLocalTrainingHandler(LocalTrainingHandler):
    def train(self, fit_params=None):
        ...
        if hasattr(self.fl_model, "is_classification") and False == self.fl_model.is_classification :
            _class_counts = None
        else :

            y_unq, y_cnts = np.unique(y, return_counts=True)
            _class_counts = {int(cls_label): int(cls_counts) for (cls_label, cls_counts) in zip(y_unq, y_cnts)}
