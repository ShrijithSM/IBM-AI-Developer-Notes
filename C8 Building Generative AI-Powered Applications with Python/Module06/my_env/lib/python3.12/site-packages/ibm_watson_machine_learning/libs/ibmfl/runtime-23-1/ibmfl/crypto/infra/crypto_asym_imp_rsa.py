...
class CryptoAsymRsa(CryptoAsym):
    ...
    def encrypt(self, plain_data: bytes) -> bytes:
        ...
        cipher_data = self.private_key.public_key().encrypt(
            plain_data,
            padding.OAEP(
                mgf=padding.MGF1(algorithm=CryptoAsymRsa.CRYPTO_HASH), algorithm=CryptoAsymRsa.CRYPTO_HASH, label=None
            ),
        )
        return cipher_data
    ...
    def get_signature(self, data: bytes) -> bytes:
        ...
        signature = self.private_key.sign(
            digest,
            padding.PSS(mgf=padding.MGF1(CryptoAsymRsa.CRYPTO_HASH), salt_length=padding.PSS.MAX_LENGTH),
