...
class CryptoKeysProtoParty:
    ...
    def __verify_parties_list(self, party_certs: dict, allowed_parties_info: dict):
        ...
        if cp_id not in self.cp_certs_str:
            raise KeyDistributionInputException(
                "cp_id=" + str(cp_id) + " not in self.cp_certs_str=" + str(self.cp_certs_str.keys())
            )
        if allowed_parties_info["cp_cert"] != self.cp_certs_str[cp_id]:
            raise KeyDistributionInputException(
                "allowed_parties_info['cp_cert']="
                + str(allowed_parties_info["cp_cert"])
                + " != self.cp_certs_str[cp_id]="
                + str(self.cp_certs_str[cp_id])
            )
        # Verify the cp_cert.
        ...
        for pid in party_certs.keys():
            if pid not in allowed_pids:
                raise KeyDistributionInputException(
                    "pid=" + str(pid) + " not in allowed_pids=" + str(allowed_parties_info["ids"])
                )
        return
    ...
    def parse_keys(self, keys_msg: dict) -> bool:
        ...
        if not ver:
            raise KeyDistributionVerificationException(
                "Invalid generating party certificate=" + str(keys_msg["gen_cert"])
            )
        ...
        if not ver:
            raise KeyDistributionVerificationException(
                "Invalid signature on he_pr_enc=" + str(keys_msg["he_pr_enc_sign"])
            )
