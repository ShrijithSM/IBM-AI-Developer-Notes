...
from ibm_watson_machine_learning.utils import print_text_header_h1, print_text_header_h2, TRAINING_RUN_DETAILS_TYPE, \
    group_metrics, StatusLogger
...
class Training(WMLResource):
        ...
        **Example**
        ...
        if details is not None:
            ...
            raise WMLClientError(
                u'Getting trained model status failed. Unable to get model details for training_uid: \'{}\'.'.format(
                    training_uid))
    ...
    def get_details(self, training_uid=None, limit=None, asynchronous=False, get_all=False, training_type=None,
        ...
        **Examples**
        ...
        if training_uid is None:
            ...
            if not _internal and details.get('entity', {}).get('status', {}).get('feature_engineering_components',
                                                                                 {}).get('obm'):
                if self._client.CPD_version >= 4.6:
    ...
    def run(self, meta_props, asynchronous=True):
        ...
        **Examples**
        ...
        if self.ConfigurationMetaNames.PROMPT_TUNING in meta_props:
            training_configuration_metadata["prompt_tuning"] = meta_props[self.ConfigurationMetaNames.PROMPT_TUNING]
        ...
        if self._client.CLOUD_PLATFORM_SPACES or self._client.ICP_PLATFORM_SPACES:
            ...
            if self.ConfigurationMetaNames.PIPELINE_UID in meta_props:
                ...
                if self.ConfigurationMetaNames.PIPELINE_DATA_BINDINGS in meta_props:
                    training_configuration_metadata["pipeline"]["data_bindings"] = meta_props[
                        self.ConfigurationMetaNames.PIPELINE_DATA_BINDINGS]
            ...
            if self.ConfigurationMetaNames.TRAINING_LIB_UID in meta_props:
                if self._client.CAMS:
                    ...
                    training_configuration_metadata["training_lib"]["href"] = {
                        "href": "/v4/libraries/" + meta_props[self.ConfigurationMetaNames.TRAINING_LIB_UID]}
                ...
                training_configuration_metadata["training_lib"].update(
                    {"command": meta_props[self.ConfigurationMetaNames.COMMAND]})
                training_configuration_metadata["training_lib"].update({"runtime": {
                    "href": "/v4/runtimes/" + meta_props[self.ConfigurationMetaNames.TRAINING_LIB_RUNTIME_UID]}})
                if self.ConfigurationMetaNames.TRAINING_LIB_MODEL_TYPE in meta_props:
                    training_configuration_metadata["training_lib"].update(
                        {"model_type": meta_props[self.ConfigurationMetaNames.TRAINING_LIB_MODEL_TYPE]})
                if self.ConfigurationMetaNames.COMPUTE in meta_props:
                    training_configuration_metadata["training_lib"].update(
                        {"compute": meta_props[self.ConfigurationMetaNames.COMPUTE]})
                if self.ConfigurationMetaNames.TRAINING_LIB_PARAMETERS in meta_props:
                    training_configuration_metadata["training_lib"].update(
                        {"parameters": meta_props[self.ConfigurationMetaNames.TRAINING_LIB_PARAMETERS]})
            ...
            if self.ConfigurationMetaNames.TRAINING_LIB in meta_props:
                ...
                if self._is_model_definition_url(meta_props[self.ConfigurationMetaNames.TRAINING_LIB]['href']) is False:
                    if ('command' not in meta_props[self.ConfigurationMetaNames.TRAINING_LIB].keys() or
                        ...
                        raise WMLClientError(
                            u'Invalid input. command, href, runtime are mandatory parameter for training_lib')
            ...
            if self._client.CAMS:
                if self._client.default_space_id is not None:
                    ...
                    training_configuration_metadata['project'] = {
    ...
    def list_subtrainings(self, training_uid):
        ...
        **Example**
        ...
        if self._client.CLOUD_PLATFORM_SPACES or self._client.ICP_PLATFORM_SPACES:
            raise WMLClientError(
                "Sub-trainings are no longer available for trainings, please use list_intermediate_models().")
        ...
        if (training_state == 'completed'):
            ...
            if 'metrics' in details[u'entity'][u'status'] and details[u'entity'][u'status'].get('metrics') is not None:
                ...
                for ml in metrics_list:
                    ...
                    new_list.append(
                        [name, path, accuracy, F1Micro, F1Macro, F1Weighted, logLoss, PrecisionMicro, PrecisionMacro,
                         PrecisionWeighted, RecallMicro, RecallMacro, RecallWeighted, createdAt])
                    new_list.append([])

                from tabulate import tabulate
                header = [u'NAME', u'PATH', u'Accuracy', u'F1Micro', u'F1Macro', u'F1Weighted', u'LogLoss',
                          u'PrecisionMicro', u'PrecisionMacro', u'PrecisionWeighted', u'RecallMicro', u'RecallMacro',
                          u'RecallWeighted', u'CreatedAt']
        ...
        **Example**
        ...
        if not self._ICP and not self._client.CLOUD_PLATFORM_SPACES and not self._client.ICP_PLATFORM_SPACES:
            response_delete = requests.delete(
                self._client.service_instance._href_definitions.get_training_href(training_uid),
    ...
    def _COS_logs(self, run_uid, on_start=lambda: {}):
        ...
        if 'connection' in run_details["entity"]["results_reference"] and run_details["entity"][
            "results_reference"].get("connection") is not None:
            ...
            try:
                if self._client.CLOUD_PLATFORM_SPACES:
                    ...
                    try:
                        key = "data/" + run_details["metadata"].get("id", run_details["metadata"].get(
                            'guid')) + "/pipeline-model.json"
                    ...
                    if pipeline_model is not None:
                        key = pipeline_model["pipelines"][0]["nodes"][0]["parameters"][
                                  "model_id"] + "/learner-1/training-log.txt"
                    else:
    ...
    def _COS_metrics(self, run_uid, on_start=lambda: {}):
        ...
        try:
            if self._client.CLOUD_PLATFORM_SPACES:
                ...
                try:
                    key = run_details["metadata"].get("id",
                                                      run_details["metadata"].get('guid')) + "/pipeline-model.json"
                    ...
                    if ex.response['Error']['Code'] == 'NoSuchKey':
                        ...
                        print(ex)
                        return
                key = pipeline_model["pipelines"][0]["nodes"][0]["parameters"].get[
                          "model_id"] + "/learner-1/evaluation-metrics.txt"
    ...
    def monitor_logs(self, training_uid):
        ...
        **Example**
        ...
        self._simple_monitor_logs(training_uid, lambda: print_text_header_h1(
            u'Log monitor started for training run: ' + str(training_uid)))
        ...
        if (status == "completed" or status == "error" or status == "failed" or status == "canceled"):
            ...
            try:
                websocket.add_header(bytes("Authorization", "utf-8"),
                                     bytes("Bearer " + self._client.service_instance._get_token(), "utf-8"))
            except:
                websocket.add_header(bytes("Authorization"),
