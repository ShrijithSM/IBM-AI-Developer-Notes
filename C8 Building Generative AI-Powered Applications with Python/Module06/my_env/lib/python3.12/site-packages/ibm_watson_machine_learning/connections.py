...
class Connections(WMLResource):
    ...
    def get_write_mode_by_datasource_type(self, datasource_type):
        Connections._validate_type(datasource_type, u'datasource_type', str, False)
        ...
        with self.requests_retry_session() as sess:
            response = sess.get(self._client.service_instance._href_definitions.get_connection_data_type_by_id_href(datasource_type),
                                params=params,
                                headers=header_param)

            datasource_details = self._handle_response(200, u'get datasource details', response)
