...
class APIClient:
    ...
    def __init__(self, wml_credentials, project_id=None, verify=None):
        ...
        if 'url' not in self.wml_credentials:
            url = BaseDataConnection._get_url_from_environment()
            if url:
                self.wml_credentials['url'] = url
                wml_credentials['url'] = self.wml_credentials['url']
            else:
        ...
        if 'instance_id' not in self.wml_credentials.keys():
            ...
            if 'icp' == self.wml_credentials[u'instance_id'].lower() or 'openshift' == self.wml_credentials[
                ...
                if 'version' not in self.wml_credentials:
                    raise WMLClientError(
                        Messages.get_message(CPDVersion.supported_version_list, message_id="version_not_provided"))
                ...
                if self.wml_credentials[u'version'].lower() in CPDVersion.supported_version_list:
                    ...
                    if 'bedrock_url' not in self.wml_credentials and self.CPD_version:
                    ...
                    if response_get_instance.status_code == 200:
                        if 'plan' in response_get_instance.json()[u'entity']:
                            ...
                            if not self.CLOUD_PLATFORM_SPACES:
                                ...
                                print(
                                    Messages.get_message(message_id="creating_assets_using_v4_beta_flow_is_deprecated"))
                        ...
                        raise WMLClientError(
                            Messages.get_message(message_id="not_authorized_to_access_the_instance_id"))
                    else:
        ...
        if self.CLOUD_PLATFORM_SPACES or self.ICP_PLATFORM_SPACES:
            ...
            self.deployments = _check_if_import_from_watsonx_ai(self.__module__,
                                                                'ibm_watson_machine_learning.deployments',
                                                                'Deployments')(self)
        ...
        self._models = _check_if_import_from_watsonx_ai(self.__module__,
                                                        'ibm_watson_machine_learning.models',
                                                        'Models')(self)
    ...
    def _get_headers(self, content_type='application/json', no_content_type=False, wsdconnection_api_req=False,
                     zen=False, projects_token=False):
        if self.WSD:
            headers = {'X-WML-User-Client': 'PythonClient'}
            ...
            if self.proceed is True:
                token_to_use = self.wml_credentials[
                    'projects_token'] if projects_token and 'projects_token' in self.wml_credentials else \
                    self.wml_credentials["token"]
            ...
            # Cloud Convergence
            if self._is_IAM() or (
                    self.service_instance._is_iam() is None and not self.CLOUD_PLATFORM_SPACES and not self.ICP_PLATFORM_SPACES):
